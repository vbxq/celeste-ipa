...
def _patch(data: bytearray, offset: int, dylib: bytes, strip_sig: bool) -> None:
    ...
    if magic != MH_MAGIC_64:
        raise SystemExit(
            f"[insert_dylib] unexpected magic 0x{magic:08x} at offset 0x{offset:x}"
        )
    ...
    if len(lc) > free:
        raise SystemExit(
            f"[insert_dylib] insufficient padding 0x{offset:x}: "
            f"{free} dispos, {len(lc)} required"
        )
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="inject an LC_LOAD_DYLIB in a Mach-O binary"
    )
    ap.add_argument("--strip-codesig", action="store_true",
                    help="remove LC_CODE_SIGNATURE (necessary before ldid -S)")
    ap.add_argument("--inplace", action="store_true",
                    help="overwrite binary instead of writing <binary>_patched")
    ap.add_argument("dylib", help="path @rpath/xxx.dylib to add")
    ap.add_argument("binary", help="target Mach-O ")
    args = ap.parse_args()
